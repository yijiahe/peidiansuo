...
public class SqliteDbHelper
{
    ...
    public void OpenDB(string connectionString)
    {
        try
        {
            ...
            dbConnection.Open();
            //dbConnection.ChangePassword("yijiahe");
            Debug.Log("Connected to db");
        }
        catch (Exception e)
        {
            string temp1 = e.ToString();
            Debug.Log("Exception:" + temp1);
        }
    }
    /// <summary>
    /// 关闭连接
    ...
    /// <returns></returns>
    public SqliteDataReader InsertClassifyInto(string tableName, string name, int parentID, int numpos)
    {
        string query = "INSERT INTO " + tableName + " VALUES (" + "null" + "," + "'" + name + "'" + "," + parentID + "," + numpos;
    ...
    /// <param name="selectvalue">要查询的字段值</param>
    /// <returns></returns>
    public SqliteDataReader UpdateClassifyInto(string tableName, string classify_name,
      int parentID, int numpos, string selectkey, string selectvalue)
    {
        //string query = "UPDATE " + tableName + " SET " + classify_name + " = " + colsvalues[0];
        //for (int i = 1; i < colsvalues.Length; ++i)
        //{
        //    query += ", " + cols[i] + " =" + colsvalues[i];
        //}
    ...
    public SqliteDataReader CreateTable(string name, string[] col, string[] colType)
    {
        ...
        query += ")";
        Debug.Log("query:" + query);
        // CREATE TABLE mytable (id int, System.String[] System.String[], System.String[] System.String[])
        return ExecuteQuery(query);
    }
    /// <summary>
    /// 根据查询条件 动态查询数据信息
